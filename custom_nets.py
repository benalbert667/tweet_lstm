...
class LayeredLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, norm_lstm=True):
        ...
        self.lstms = nn.ModuleList(
            [LSTM1(self.i_s, self.h_s) if norm_lstm else LSTM2(self.i_s, self.h_s)
             for _ in range(self.n_layers)])

    def init_new_seq(self, random=False):
        for i in range(self.n_layers):
            self.hidden_states[i], self.cell_states[i] = self.lstms[i].init_states(random=random)
...
class LSTM2(LSTM1):
    def __init__(self, input_size, hidden_size):
        super(LSTM2, self).__init__(input_size, hidden_size)
        # new gates are each a 3-layered feedforward net
